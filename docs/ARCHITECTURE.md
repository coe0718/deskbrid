...
pub async fn create_backend(
    ...
    // Currently only GNOME is implemented
...
- On success: stores the backend in `DaemonState.backend`, logs "GNOME backend loaded successfully"
- On failure: logs a warning, continues without desktop features. All desktop actions return `NOT_SUPPORTED` errors

The trait is designed to be implemented for other desktop environments (KDE, Sway, Hyprland, etc.) by adding a new backend module and updating `create_backend()`.
...
├── cli.rs          — CLI argument parsing (subcommands: daemon, status, stop, restart, install)
├── client.rs       — Embedded client mode (reads NDJSON from stdin, sends to daemon)
├── capture.rs      — Screenshot helpers (grim/slurp invocation, PNG dimension extraction)
└── backend/
    ├── mod.rs      — DesktopBackend trait definition, create_backend() factory
