...
pub(super) async fn files_watch(
    ...
    backend.watchers.lock().await.insert(watched_path, watcher);
    Ok(())
}

pub(super) async fn files_unwatch(backend: &CosmicBackend, path: &str) -> anyhow::Result<()> {
    backend.watchers.lock().await.remove(path);
    Ok(())
}
