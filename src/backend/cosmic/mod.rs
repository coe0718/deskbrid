...
impl CosmicBackend {
    pub async fn new(
        ...
        let wl_socket = match std::env::var("WAYLAND_DISPLAY") {
            Ok(s) => Some(s),
            Err(_) => detect_wayland_socket(&xdg_runtime).await,
        };
...
async fn detect_wayland_socket(xdg_runtime: &str) -> Option<String> {
