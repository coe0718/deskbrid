...
impl DesktopBackend for CosmicBackend {
    ...
    // ─── Monitor Control (not yet supported on COSMIC) ───────────
    async fn monitor_set_primary(&self, _output: &str) -> anyhow::Result<()> {
        anyhow::bail!("COSMIC does not expose a primary monitor setting")
    }
    ...
    async fn monitor_set_resolution(
        ...
        anyhow::bail!("monitor resolution changes not yet supported on COSMIC")
    }

    async fn monitor_set_scale(&self, _output: &str, _scale: f64) -> anyhow::Result<()> {
        anyhow::bail!("monitor scale changes not yet supported on COSMIC")
    }

    async fn monitor_set_rotation(&self, _output: &str, _rotation: &str) -> anyhow::Result<()> {
        anyhow::bail!("monitor rotation changes not yet supported on COSMIC")
    }

    async fn monitor_set_enabled(&self, _output: &str, _enabled: bool) -> anyhow::Result<()> {
        anyhow::bail!("monitor enable/disable not yet supported on COSMIC")
    }
}
