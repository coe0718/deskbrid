...
impl HyprBackend {
    ...
    pub(super) async fn monitor_config(&self, output: &str) -> anyhow::Result<super::free_functions::HyprMonitorConfig> {
        let json = match self.hyprctl_json(&["monitors", "all"]).await {
            Ok(json) => json,
            Err(_) => self.hyprctl_json(&["monitors"]).await?,
        };
        let arr = json.as_array().ok_or_else(|| anyhow::anyhow!("expected array"))?;
        let monitor = arr.iter()
        ...
        Ok(super::free_functions::HyprMonitorConfig {
            ...
            width: monitor.get("width").and_then(|v| v.as_u64()).unwrap_or(1920) as u32,
            height: monitor.get("height").and_then(|v| v.as_u64()).unwrap_or(1080) as u32,
            refresh_rate: monitor.get("refreshRate").and_then(|v| v.as_f64()).unwrap_or(60.0),
            ...
            transform: monitor.get("transform").and_then(|v| v.as_i64()).unwrap_or(0) as i32,
        })
    }

    pub(super) async fn apply_monitor_config(&self, config: &super::free_functions::HyprMonitorConfig) -> anyhow::Result<()> {
        let value = format!(
            "{},{}x{}@{},{}x{},{},transform,{}",
    ...
    pub(super) fn hyprctl_client_to_window(c: &serde_json::Value) -> protocol::WindowInfo {
        ...
        protocol::WindowInfo {
            ...
            app_id: c.get("class").and_then(|v| v.as_str()).unwrap_or("").to_string(),
            workspace_id: c.get("workspace").and_then(|v| v.get("id")).and_then(|v| v.as_i64()).unwrap_or(1) as u32,
            is_focused: c.get("focusHistoryID").and_then(|v| v.as_i64()).unwrap_or(-1) == 0,
            is_minimized: false,
    ...
    pub(super) async fn window_is_fullscreen(&self, id: &str) -> anyhow::Result<bool> {
        ...
        let active_id = window.get("address").and_then(|v| v.as_str()).unwrap_or_default();
        if !active_id.eq_ignore_ascii_case(id) { return Ok(false); }
    ...
    pub(super) async fn idle_seconds_inner(&self) -> anyhow::Result<u64> {
        ...
        if let Ok(entries) = std::fs::read_dir("/dev/input") {
            for entry in entries.flatten() {
                ...
                {
                    let secs = ts.as_secs();
                    if secs > newest && secs <= now { newest = secs; }
                }
            }
        }
        if newest > 0 { Ok(now.saturating_sub(newest)) } else { Ok(0) }
    }
}
