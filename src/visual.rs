...
pub(crate) fn diff_images(
    ...
    let bbox = (changed_pixels > 0).then(|| BoundingBox {
        x: min_x,
        y: min_y,
        width: max_x - min_x + 1,
...
pub use vision::{
    VisionDetectStateRequest, VisionFindByTextRequest, VisionFindElementRequest,
    vision_detect_state, vision_find_by_text, vision_find_element,
};
...
mod tests {
    ...
    fn detects_changed_bounding_box() {
        ...
        assert_eq!((bbox.x, bbox.y, bbox.width, bbox.height), (2, 1, 1, 1));
    }

    #[test]
    fn ignores_equal_images() {
        let image = DynamicImage::ImageRgba8(ImageBuffer::from_pixel(2, 2, Rgba([1, 2, 3, 255])));
        let (diff, _) = diff_images(&image, &image, 0).unwrap();
