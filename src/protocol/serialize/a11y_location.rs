...
pub(super) fn serialize_a11y_location(action: &Action, id: &str) -> serde_json::Value {
    match action {
        ...
        Action::UiElementClick { selector, tab_index } => {
        ...
        Action::UiElementSetText { selector, text, tab_index } => {
            let mut v = json!({"type":"ui.element.set_text","id":id,"selector":selector,"text":text});
            if let Some(ti) = tab_index {
                v["tab_index"] = json!(ti);
            }
            v
