...
- `src/mcp/` — MCP server: 100+ rmcp tools over stdio (`deskbrid mcp`) and TCP (`deskbrid daemon --mcp-port`)
- `src/protocol/` — 100+ NDJSON action types: windows, input, clipboard, audio, system, files, browser/CDP, MPRIS, process, terminal, monitor, notifications, hotkeys, layouts, bluetooth, network, confirmation, agent messaging, unified search
- `site/` — landing page + install.sh
- `extensions/` — GNOME Shell extension
- `hermes/` — Hermes agent plugin with bundled skill (100+ tools) and direct Unix socket integration
